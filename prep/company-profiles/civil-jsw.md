# JSW — Civil Placement Strategy

> **Source of truth:** [`placement_data.csv`](../../../Civil_Placement_IITK/placement_data.csv) row 234.

## 1. Company Snapshot

| Field | Detail |
|---|---|
| **Company** | JSW Group |
| **Civil Domain(s)** | Construction & Infrastructure; Materials (steel/cement); Ports; Power infrastructure; Mining |
| **Role** | Graduate Engineer Trainee |
| **CTC** | CTC ₹13,00,000 (₹10,40,000 fixed + ₹2,60,000 variable) + ₹2,00,000 retention (over 2 yrs); post-confirmation ₹16,00,000 `[CSV]` |
| **Location** | PAN India `[CSV]` |
| **Relevance** | **MEDIUM** — diversified infrastructure/steel/cement/ports group; generic GET open to civil `[INFERRED]` |

## 2. Role Reverse Engineering

| Job Responsibility | Required Knowledge | Preparation Action |
|---|---|---|
| Construction execution (infrastructure) | RCC design, steel structures, construction tech | Study IS codes + construction methods |
| Project management | Scheduling, estimation, cost control | PM fundamentals (CPM, PERT) |
| Steel/cement plant operations | Industrial construction, materials science | Understand steel manufacturing basics |
| Port/mining infrastructure | Geotechnical, transportation, marine basics | Study port construction + mining civil works |

## 3. Technical Syllabus

| Priority | Topic | Depth |
|---|---|---|
| **P0** | Construction Technology & RCC Design | Strong |
| **P0** | Steel Structures (IS 800) | Strong |
| **P0** | Estimation & Project Management | Strong |
| **P1** | Geotechnical Engineering | Basic |
| **P1** | Transportation (ports, roads) | Basic |
| **P2** | Mining basics | Awareness |

## 5. Key Interview Questions

1. `[INFERRED]` What do you know about JSW's infrastructure and steel/cement businesses?
2. How does steel manufacturing relate to civil engineering?
3. Explain the construction process for a port/terminal structure.
4. What are the different types of construction projects JSW undertakes?
5. Describe a situation where you worked in a team under tight deadlines.
6. What is your understanding of project lifecycle management?

## 5. Preparation (7-Day)

| Day | Focus |
|---|---|
| 1 | RCC Design — IS 456 fundamentals, beam/column design |
| 2 | Steel Structures — IS 800 basics, connections, tension members |
| 3 | Construction Technology — industrial construction methods |
| 4 | Estimation & Project Management — CPM, PERT, cost control |
| 5 | Infrastructure basics — ports, roads, mining civil works |
| 6 | JSW company research — businesses, recent projects, values |
| 7 | Mock interview + behavioral prep |

## 6. Strategy Card

```
COMPANY: JSW Group
TARGET ROLE: Graduate Engineer Trainee
CIVIL DOMAIN: Infrastructure, Steel, Cement, Ports, Mining
RELEVANCE: MEDIUM (branch-agnostic GET; infrastructure group)
CTC: ₹13-16L

TOP 5 AREAS:
1. RCC + Steel design
2. Construction technology
3. Project management
4. Infrastructure knowledge (ports, roads)
5. JSW business awareness

7-DAY PRIORITY:
Day 1-2: RCC/Steel design | Day 3: Construction tech | Day 4: Estimation
Day 5: Port/mining basics | Day 6: JSW research | Day 7: Mock
```

## Cross-Links
- [RCC Design](../../core/rcc/rcc-design.md)
- [Steel Design](../../core/steel/steel-design.md)

## References
- JSW Group Corporate website (www.jsw.in)
- [`placement_data.csv`](../../../Civil_Placement_IITK/placement_data.csv) — Row 234
